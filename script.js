...
class AdminManager {
                ...
                <tr>
                    ...
                    <td>
                        <div class="user-actions">
							<button class="btn-user-action" onclick="window.adminManager.sendMessageToUser('${user.id}')" title="Send Message">✉️</button>
    ...
    const message = prompt(`Tulis mesej kepada ${user.name}:`);
    if (!message) return;
    ...
    window.notificationManager.showNotification(
        'Mesej Dihantar',
        `Mesej telah dihantar kepada ${user.name}.`,
        'success'
    );
}
...
// ⬇️ LETAKKAN KOD INI DI SINI — SELEPAS else BLOCK TUTUP
const messageData = localStorage.getItem(`messages_${this.currentUser.id}`);
if (messageData) {
    const { message } = JSON.parse(messageData);
    window.notificationManager.showNotification("Pesanan dari Admin", message, "info");
...
if (reportBtn) {
    ...
    if (!this.checklists || this.checklists.length === 0) {
        window.notificationManager.showNotification('No Data', 'You have no checklists to report.', 'warning');
        return;
    }

    let csvContent = "data:text/csv;charset=utf-8,";
    csvContent += "Title,Subject,Priority,Due Date,Tasks,Completed\n";

    this.checklists.forEach(cl => {
        const totalTasks = cl.tasks.length;
        const completedTasks = cl.tasks.filter(t => t.completed).length;
        const dueDate = cl.dueDate ? new Date(cl.dueDate).toLocaleString() : "N/A";
        const sanitizedTitle = cl.title.replace(/,/g, ' ');
        csvContent += `${sanitizedTitle},${cl.subject},${cl.priority},${dueDate},${totalTasks},${completedTasks}\n`;
    });

    const encodedUri = encodeURI(csvContent);
    const link = document.createElement("a");
    link.setAttribute("href", encodedUri);
    link.setAttribute("download", "checklist_report.csv");
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);

    window.notificationManager.showNotification('Report Generated', 'Your checklist report has been downloaded.', 'success');
}
